...
function areacodes_regions_menu() {
		...
		add_submenu_page( 'areacodes-manager', 'US States', 'US States', 'read', 'edit-tags.php?taxonomy=ms_areacodes_usstates' );
}
...
function menu_highlight( $parent_file ) {
	...
	if ( 'ms_areacodes_regions' === $taxonomy || 'ms_areacodes_usstates' === $taxonomy ) {
...
add_action(
	...
	function () {
		...
		register_taxonomy( 'ms_areacodes_regions', array( 'ms_countries' ), $args );
	},
	0
);
...
add_action(
	...
	function () {
		$labels = array(
			'name'          => _x( 'US States', 'Taxonomy General Name', 'ms' ),
			'singular_name' => _x( 'US State', 'Taxonomy Singular Name', 'ms' ),
		...
		register_taxonomy( 'ms_areacodes_usstates', array( 'ms_areacodes' ), $args );
	},
	0
);
