...
const ReviewSelect = ( props ) => {
	...
	const { categoryId, reviewId, reviewMeta } = attributes;
	...
	useSelect( ( select ) => {
		if ( reviewId ) {
			const review = select( 'core' ).getEntityRecord( 'postType', 'ms_reviews', reviewId );
			if ( review ) {
				setAttributes( { reviewMeta: review.meta } );
			}
