...
// if ( $_GET ) {
	...
	if ( empty( $visitor_email ) ) {
			echo json_encode( array( 'response' => 'ERROR', 'message' => 'Email is mandatory!' ) );
			exit;
	}
	
	if ( is_injected( $visitor_email ) ) {
			echo json_encode( array( 'response' => 'ERROR', 'message' => 'Bad email value!' ) );
			exit;
	}
...
function create_api_ticket( $post_data ) {
	...
	if( $server_output->message ) {
		...
		echo json_encode( array( 'response' => 'ERROR', 'message' => $server_output->message ) );
	} else {
		// success response
