...
export const compareResults = ( alternative, agentsFromSlider, selFeature ) => {
	...
	const unsupportedFeatures = getWebCalcData().filter(
	...
	const laPrice = getPriceAgents( 'liveagent', agentsFromSlider, selFeature ).price;
	const altPrice = getPriceAgents( alternative, agentsFromSlider, selFeature ).price;

	// Gets current Alternative agents based on slider, if alternative has fixed agents, returns biggest fixed value
	const altAgents = getPriceAgents( alternative, agentsFromSlider, selFeature ).agents;
...
export const getMostExpensive = ( agentsFromSlider, selFeature ) => {
	const selectedPrices = alternatives.map(
		( alternative ) => {
	...
	const getAlternatives = alternatives.map(
		( alternative ) => {
			if ( typeof alternative === 'string' ) {
				return getPriceAgents( alternative, agentsFromSlider, selFeature );
			}
			// If undefined, returns 0
			return 0;
		}
