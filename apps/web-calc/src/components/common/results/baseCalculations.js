...
// We get selected alternatives from session storage
...
export const getPriceAgents = (alternative, agentsFromSlider) => {
  const packagePrice = getPackage(alternative).price;
  const packageAgents = getPackage(alternative).agents;
...
export const getChartWidth = (alternative, agentsFromSlider, maxprice) => (
  (getPriceAgents(alternative, agentsFromSlider).price / maxprice)
		* 100
);

// By Selected Features it attempts to find the lowest package that has the requested feature
export function getPackage(alternative) {
  const prices = getSelectedFeatures().map((feature) => {
  ...
  const packages = getSelectedFeatures().map((feature) => {
    const thisFeature = data[alternative].features[feature];
    if (typeof thisFeature === 'string') {
...
// We get all the selected features from the sessionStorage
export function getSelectedFeatures() {
  const keys = Object.keys(getWebCalcData);
  const trueOnly = keys.filter((key) => (key !== ('ispreset' || 'waspreset') && getWebCalcData[key] === true));
  return trueOnly;
}

export default getSelectedFeatures;

// Getting selected features with names
export function getSelectedFeaturesNamed() {
  const array = [];
...
export const getMaxPrice = (alternatives, agentsFromSlider) => {
  ...
  selectedPrices.push(
    getPriceAgents('liveagent', agentsFromSlider).price,
  );
